...
Sample Tempareture Check


```{r}

Bio_s_t <- Bio_data %>%
  select(fcn_no, sample_temp, sample_temp_adult, arrival_temperature, arrival_temp_room, respondent_cat, survey_date,saliva_amount_adult,saliva_amount,saliva_amount_a,saliva_amount_b) %>%
  mutate(
    sample_temp_all = ifelse(!is.na(sample_temp), sample_temp, sample_temp_adult)
  ) %>%
  filter(sample_temp_all > 0, sample_temp_all < 30) %>%
  mutate(
    sample_temp_Category = ifelse(sample_temp_all <= 8, "A", "B"),
    date = mdy(survey_date),      # Convert to Date
...
Bio_s_t <- Bio_s_t %>%
  mutate(
    saliva_amount_all = ifelse(
      !is.na(saliva_amount) | !is.na(saliva_amount_a) | !is.na(saliva_amount_b),
      rowSums(across(c(saliva_amount, saliva_amount_a, saliva_amount_b)), na.rm = TRUE),
      saliva_amount_adult
    )
  )
...
Bio_s_t %>% 
  filter(sample_temp_Category == "B") %>% 
  View()


Bio_s_t_B<-Bio_s_t %>% filter(sample_temp_Category == "B")
...
ggplot(Bio_s_t_B, aes(x = as.factor(Month), fill = as.factor(Year))) +
  ...
  labs(
    title = "Month and Year-wise Distribution of Samples",
    x = "Month",
    y = "Count",
    fill = "Year"
  ) +
...
Bio_s_t<-Bio_s_t %>% filter(saliva_amount_all<20) %>% filter(saliva_amount_all>0)





Bio_s_M<-Bio_s_t %>% filter(respondent_cat==1 |respondent_cat==4|respondent_cat==6| respondent_cat==7 )


Bio_s_F<-Bio_s_t %>% filter(respondent_cat==2  )

Bio_s_C<-Bio_s_t %>% filter(respondent_cat==61 |respondent_cat==71 )



summary(Bio_s_M$saliva_amount_all)
summary(Bio_s_F$saliva_amount_all)
summary(Bio_s_C$saliva_amount_all)
...
```{r}
#### Saliva
Saliva_Succesfull_check<-Bio_data %>% select(saliva_amount,saliva_amount_a,saliva_amount_b,respondent_cat,fcn_no) %>% filter(!is.na(saliva_amount_a)) 

Mother <- Saliva_Succesfull_check %>%
  filter(respondent_cat %in% c(1, 4, 6, 7)) %>%
  filter(!is.na(saliva_amount_b))

#### Buccal swab


Buccal_Succesfull_check<-Bio_data %>% select(buccal_sample_ID_1,buccal_sample_ID_2,respondent_cat,fcn_no,restype) %>% filter(buccal_sample_ID_1!="") %>% filter(buccal_sample_ID_1!="999")


table(Buccal_Succesfull_check$respondent_cat,Buccal_Succesfull_check$buccal_sample_ID_2)

table(Buccal_Succesfull_check$respondent_cat)

#count(Buccal_Succesfull_check$buccal_sample_ID_2)
...
temp_data <- data.frame(
  Stage = c("Immediately after collection", "Upon arrival at laboratory"),
  Maintained = c(88.89, 84.91),
  Not_Maintained = c(11.11, 15.09)
)

# Transform data into long format for ggplot
temp_long <- temp_data %>%
  pivot_longer(cols = c(Maintained, Not_Maintained),
               names_to = "Condition",
               values_to = "Percentage")

# Plot bar chart
ggplot(temp_long, aes(x = Stage, y = Percentage, fill = Condition)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(Percentage, "%")), 
            position = position_dodge(width = 0.9), 
            vjust = -0.3, size = 3.5) +
  scale_fill_manual(values = c("#1b9e77", "#d95f02"),
                    labels = c("Maintained", "Not Maintained")) +
  theme_minimal(base_size = 13) +
  labs(
    title = "Sample Collection Temperature Maintenance",
    x = "Stage",
    y = "Percentage (%)",
    fill = "Temperature Condition"
  ) +
  ylim(0, 100)
...
# Create the data frame
data <- data.frame(
  Stage = c("Immediately after collection", "Immediately after collection",
            "Upon arrival at laboratory", "Upon arrival at laboratory"),
  Temperature_Range = c("2-8°C", ">8°C", "2-8°C", ">8°C"),
  Count = c(13586, 1698, 12978, 2306),
  Percentage = c(88.89, 11.11, 84.91, 15.09)
)
...
ggplot(data, aes(x = Stage, y = Count, fill = Temperature_Range)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_text(aes(label = paste0(format(Count, big.mark = ","), " (", Percentage, "%)")),
            position = position_dodge(width = 0.7),
            vjust = -0.3, size = 3.5, fontface = "bold") +
  scale_fill_manual(values = c("2-8°C" = "#2E8B57",  # Green for proper temperature
                              ">8°C" = "#DC143C"),   # Red for exceeded temperature
                    name = "Temperature Range") +
  labs(title = "Sample Temperature Maintenance During Collection and Transport",
       subtitle = "Comparison between immediate collection and laboratory arrival stages",
       x = "Collection Stage",
       y = "Number of Samples") +
  theme_minimal() +
...
ggplot(data, aes(x = Stage, y = Count, fill = Temperature_Range)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
        plot.subtitle = element_text(hjust = 0.5, size = 10),
        axis.text.x = element_text(angle = 0, hjust = 0.5, size = 10), # Changed angle to 0
        legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  scale_y_continuous(labels = comma, expand = expansion(mult = c(0, 0.1)))
...
# Ensure correct factor order for legend
data$Temperature_Range <- factor(data$Temperature_Range, levels = c("2-8°C", ">8°C"))
