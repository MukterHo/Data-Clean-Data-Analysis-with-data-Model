...
PSS_data<-HH_survey_data %>% select(fcn_no,respondent_cat,pss_1,pss_2,pss_3,pss_4,pss_5,pss_6,pss_7,pss_8,pss_9,pss_10,pss_11) %>% filter(!is.na(pss_1))
...
```{r}
Aspiration_data<-HH_survey_data %>% filter(respondent_cat==3) %>% select(fcn_no,aspiration_1,aspiration_2)

PSS_data1<-PSS_data %>% filter(respondent_cat==3) %>% full_join(Aspiration_data,by="fcn_no")
...
```{r}
PSS_data1<-PSS_data1 %>% filter(aspiration_1!=88)
...
table(PSS_data1$aspiration_1,PSS_data1$PSS_total)
```
